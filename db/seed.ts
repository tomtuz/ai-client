...
function insertUserConfig(config) {
  return new Promise((resolve, reject) => {
    db.run(
      "INSERT INTO UserConfig (key, value) VALUES (?, ?)",
...
function insertMessage(conversationId: string, messageData: MessageContents) {
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO Messages (api_message_id, conversation_id, type, role, model) 
            VALUES (?, ?, ?, ?, ?)`,
      [
        messageData.id,
        conversationId,
      ...
      function (err) {
        if (err) {
          ...
          const contentPromises = messageData.content.map((content) => {
            return db.run(
              `INSERT INTO MessageContents (message_id, content_type, content_text) 
                           VALUES (?, ?, ?)`,
          ...
          // Insert token usage
          const tokenPromise = db.run(
            `INSERT INTO TokenUsage (message_id, input_tokens, output_tokens) 
                                       VALUES (?, ?, ?)`,
